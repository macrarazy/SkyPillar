...
var commands = exports.commands = {
	...
	roomhelp: function (target, room, user) {
		...
		this.sendReplyBox(
			...
			"<br />" +
			"Room Manager (#) can also use:<br />" +
			"- /roomowner, /roomowner <em>username</em>: appoint a person to room owner<br />" +
			"- /roomdeowner, /roomdeowner <em>username</em>: remove a room owner<br />" +
			"</div>"
		);
	},
	...
	potd: function (target, room, user) {
		if (user.userid !== 'ashiemore' && user.userid !== 'rivalnick') return this.sendReply('The command \'/potd\' was unrecognized. To send a message starting with \'potd\', type \'//potd\'.');
	...
	if (!this.can('mute', null, room)) return false;
				if (room.id !== 'casino') return this.sendReply('You can only use lotto ticket in the casino room.');
		if (!this.canBroadcast()) return;
		var lt = target.indexOf("lt");
		if (lt != -1) {
			var num = parseInt(target.substring(0, lt));
			var faces;
			if (target.length > lt) faces = parseInt(target.substring(lt + 1));
			if (isNaN(num)) num = 1;
			if (isNaN(faces)) return this.sendReply("The number of faces must be a valid integer.");
			if (faces < 1 || faces > 1000) return this.sendReply("The number of faces must be between 1 and 1000");
			if (num < 1 || num > 20) return this.sendReply("The number of dice must be between 1 and 20");
			...
			var total = 0;
			for (var i = 0; i < num; ++i) {
				tickets[i] = (Math.floor(faces * Math.random()) + 1);
				total += tickets[i];
			}
			return this.sendReplyBox("Randomlly picked lotto " + num + "x(1 - " + faces + "): " + tickets.join(", ") + "<br />Total: " + total);
		}
		if (target && isNaN(target) || target.length > 31) return this.sendReply("The max roll must be a number under 30 digits.");
		var maxRoll = (target)? target : 30;
		var one = Math.floor(maxRoll * Math.random()) + 1;
		var two = Math.floor(maxRoll * Math.random()) + 1;
		var three = Math.floor(maxRoll * Math.random()) + 1;
		var four = Math.floor(maxRoll * Math.random()) + 1;
		var five = Math.floor(maxRoll * Math.random()) + 1;
		var six = Math.floor(maxRoll * Math.random()) + 1;
	...
	help: function (target, room, user) {
		...
		if (!target) {
			...
			this.sendReply("For help on lotto ticket, use /tickethelp");
			if (user.group !== Config.groupsranking[0]) {
				...
				this.sendReply("For the rules, use /staffrules");
			}
